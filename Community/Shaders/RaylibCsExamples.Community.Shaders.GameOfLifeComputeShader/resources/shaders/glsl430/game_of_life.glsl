...
layout (std430, binding = 2) readonly restrict buffer golLayout2 {
...
void main()
{
  ...
  if (neighbourCount == 3) setGol(x, y, 1);                           // Cell is born
  else if (neighbourCount < 2 || neighbourCount > 3) setGol(x, y, 0); // Cell dies
  else setGol(x, y, fetchGol(x, y));                                  // Cell survives
}
